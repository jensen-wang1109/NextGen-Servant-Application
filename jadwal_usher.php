    ...
    <div id="wrapper">
        ...
        <div class="container fluid">
            <div class="page_head" style="margin-bottom: 50px;">
                <h3 style="color: black;">Hospitality</h3>
            ...
            <div class="row" style="font-family: Poppins; float:left; margin-right:20px;" id="tambah">
                <a class="btn btn-success" href="form_jadwal_usher.php" role="button"><i class="fas fa-plus"></i>&ensp;Add Jadwal Usher</a>
            </div>
                    ...
                    <table id="table" class="table table-hover table-striped" style="color:black;">
                        <thead style="text-align: center;">
                            <tr>
                                ...
                                <th scope="col">Add/Remove Usher</th>
                                <th scope="col">Delete Jadwal Usher</th>
                        ...
                        <?php 
                            ...
                            $query = mysqli_query($con, "SELECT i.ibadah_id, DATE_FORMAT(i.tanggal_ibadah, '%d %M %Y') AS tanggal_ibadah, i.shift, GROUP_CONCAT(j.fullname_jemaat SEPARATOR ', ') AS list_usher, GROUP_CONCAT(ui.keterangan_usher SEPARATOR '|') AS ket_usher, GROUP_CONCAT(j.fullname_jemaat SEPARATOR '|') AS list_usher_2, GROUP_CONCAT(u.usher_id SEPARATOR ',') AS list_usher_id FROM usher u JOIN usher_ibadah ui ON u.usher_id = ui.usher_id JOIN ibadah i ON ui.ibadah_id = i.ibadah_id JOIN jemaat j ON j.jemaat_id = u.jemaat_id GROUP BY ui.ibadah_id, i.shift;");
                            ...
                            while($row = mysqli_fetch_array($query)) {
                                ...
                                <tr scope="row">
                                    ...
                                    <td><?php echo $row['list_usher']; ?>
                                    </td>
                                    <td style="text-align: center; width:8%;">
                                        <a href="form_add_usher.php?ibadahId=<?php echo $row['ibadah_id']; ?>" class="btn btn-success btn-sm" title="Add">
                                        <i class="fas fa-plus"></i>
                                        </a>
                                        <a href="form_remove_usher.php?ibadahId=<?php echo $row['ibadah_id']; ?>" class="btn btn-danger btn-sm" title="Remove"><i class="fas fa-minus"></i>
                                        </a>
                                    </td>
                                    <td style="text-align:center; width: 10%;">
                                        <a href="jadwal_usher_delete.php?ibadahId=<?php echo $row['ibadah_id'];?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin menghapus jadwal usher?')">
                                        Delete
                                        </a>
                                    </td>
                                    <td style="display: none;">
                                        <?php echo $row['list_usher_2']; ?>
                                    </td>
                                    <td style="display:none;">
                                        <?php echo $row['ket_usher']; ?>
                                    </td>
                                </tr>
