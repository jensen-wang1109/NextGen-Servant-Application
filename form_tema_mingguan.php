...
<?php 
    ...
    $dataTema = mysqli_query($con, "SELECT GROUP_CONCAT(i.ibadah_id) AS ibadah_id, month,
    CASE 
    ...
    END AS bulan,
    year, judul_tema_bulanan, preview_tema_bulanan
    ...
    if(isset($_POST['save'])) {
        ...
        $getTglIbadah = mysqli_query($con, "SELECT tanggal_ibadah FROM ibadah WHERE ibadah_id = '$tglIbadah'");
        ...
        //insert into tema_mingguan
        mysqli_query($con, "INSERT INTO tema_mingguan VALUES (NULL, '$temaMingguan', '$previewTemaMingguan', '$paramTemaBulananId', '$tglIbadah')");
        if(!empty($ibadahKeduaId)) {
            mysqli_query($con, "INSERT INTO tema_mingguan VALUES (NULL, '$temaMingguan', '$previewTemaMingguan', '$paramTemaBulananId', '$ibadahKeduaId')");
        }
...
<head>
    ...
    <title>| FORM TEMA MINGGUAN</title>
    ...
    <div id="wrapper">
        ...
        <div class="container fluid">
            <div class="page_head" style="margin-bottom: 50px;">
                <h3 style="color: black;">Form Tema Mingguan</h3>
                ...
                <p style="display: inline;">&ensp; / &ensp;</p>
                <a href="form_tema_mingguan.php?temaBulananId=<?php echo $paramTemaBulananId; ?>" style="display: inline;">Form Tema Mingguan</a>
            </div>

            <div class="clean"></div>

            <div class="row" style="font-family: Poppins; float:left; margin-right:20px;" id="tambah">
                <a class="btn btn-danger" href="tema_mingguan.php?temaBulananId=<?php echo $paramTemaBulananId; ?>" onclick="return confirm('Apakah yakin ingin membatalkan proses penginputan data?');"  role="button"><i class="fas fa-minus"></i>&ensp;Cancel</a>
            </div>
                ...
                <form style="color: black; padding-bottom: 50px; background-color: white;" method="POST" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="kanan" style="width: 45%; float: left; margin-left: 25px; padding-top:30px;">
                        <?php
                            ...
                            $listIbadah = mysqli_query($con, "SELECT DATE_FORMAT(tanggal_ibadah, '%d %M %Y') AS tanggal_ibadah, shift, ibadah_id FROM ibadah WHERE EXTRACT(YEAR FROM tanggal_ibadah) = '$year' AND EXTRACT(MONTH FROM tanggal_ibadah) = '$month' AND ibadah_id NOT IN ($ibadahId) GROUP BY tanggal_ibadah ORDER BY tanggal_ibadah ASC;");
                        ?>
                        ...
                        <div class="input-group mb-3" style="padding-bottom: 10px;">
                            <input type="text" class="form-control" placeholder="Masukkan Tema Mingguan" name="inputtemamingguan" style="color: black;" required>
                        </div>

                        <h6><b>Preview Tema Mingguan</b></h6>
                        <div class="input-group mb-3" style="padding-bottom: 10px;">
                            <textarea cols="50" rows="10" class="form-control" style="color: black;" placeholder="Masukkan Preview Tema Mingguan" name="inputpreviewtemamingguan" required></textarea >
                        </div>
