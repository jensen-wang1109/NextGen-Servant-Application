...
<?php 
    ...
    $edit_ibadah = mysqli_query($con, "SELECT * FROM ibadah WHERE ibadah_id = '$paramIbadahId'");
    $row_edit_ibadah = mysqli_fetch_array($edit_ibadah);

    if(isset($_POST['update'])) {
        $tglIbadah = $_POST['inputtglibadah'];
        $shiftIbadah = $_POST['selectedshift'];

        $queryCekIbadah = mysqli_query($con, "SELECT * FROM ibadah WHERE tanggal_ibadah = '$tglIbadah' AND shift = '$shiftIbadah' AND ibadah_id != '$paramIbadahId'");

        if(mysqli_num_rows($queryCekIbadah) > 0) {
            echo "<script>
                alert('Data Ibadah dengan tanggal dan shift yang sama sudah terdaftar!');
                document.location = 'form_ibadah_edit.php?ibadahId=$paramIbadahId'
            </script>";
        }
        else {
            //update data ibadah
            mysqli_query($con, "UPDATE ibadah SET tanggal_ibadah='$tglIbadah', shift='$shiftIbadah', ref_user_id='$userId' WHERE ibadah_id = '$paramIbadahId'");
    ...
    <div id="wrapper">
        ...
        <div class="container fluid">
            <div class="page_head" style="margin-bottom: 50px;">
                <h3 style="color: black;">Form Edit Ibadah</h3>
                ...
                <p style="display: inline;">&ensp; / &ensp;</p>
                <a href="form_ibadah_edit.php?ibadahId=<?php echo $paramIbadahId;?>.php" style="display: inline;">Form Edit Ibadah</a>
            </div>

            <div class="clean"></div>

            <div class="row" style="font-family: Poppins; float:left; margin-right:20px;" id="tambah">
                <a class="btn btn-danger" href="ibadah.php" onclick="return confirm('Apakah yakin ingin membatalkan proses mengedit data?');" role="button"><i class="fas fa-minus"></i>&ensp;Cancel</a>
            </div>
                ...
                <form style="color: black; padding-bottom: 50px; background-color: white;" method="POST" enctype="multipart/form-data" autocomplete="off">
                    <div class="kiri" style="width: 50%; margin-left: 20px; float:left; margin-right: 35px; padding-top:30px;">
                        ...
                        <div class="input-group mb-3" style="padding-bottom: 10px;">
                            <input type="date" class="form-control" placeholder="dd/mm/yyyy" name="inputtglibadah" value="<?php echo $row_edit_ibadah['tanggal_ibadah']; ?>" style="color: black;" required>
                        </div>
                        ...
                        <div class="shift_sempit" style="width: 45%; float:left;">
                            ...
                            <!-- <div class="form-check form-check-inline" style="margin-right: 30px;">
                                <input class="form-check-input" type="radio" name="selectedshift" id="inlineRadio1" value="1" style="cursor:pointer;" <?php if($row_edit_ibadah['shift'] == '1') { ?> checked <?php } ?> required>
                                <label class="form-check-label" for="inlineRadio1" style="cursor:pointer;color:black;">1</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="selectedshift" id="inlineRadio2" value="2" style="cursor:pointer;" <?php if($row_edit_ibadah['shift'] == '2') { ?> checked <?php } ?>>
                                <label class="form-check-label" for="inlineRadio2" style="cursor:pointer; color:black;">2</label>
                            </div> -->
                            <div class="input-group mb-3" style="padding-bottom: 10px;">
                                <input type="number" class="form-control" placeholder="Masukkan Shift Ibadah" name="selectedshift" style="color: black;" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==1) return false;" value="<?php echo $row_edit_ibadah['shift']; ?>" min="1" max="9" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <center style="background-color: white; margin-top: 50px;">
                        <input type="submit" class="btn btn-success" value="Update Data Ibadah" name="update">
                    </center>
                </form>
            </div>
        </div>
