...
<?php 
    ...
    if(isset($_POST['save'])) {
        ...
        mysqli_query($con, "INSERT INTO musik_ibadah VALUES (NULL, '$tglIbadah', '$pemusikId', '$jenisPemusik')");

        echo "<script>
            alert('Data sudah berhasil disimpan.');
            var ask = confirm('Apakah ingin add pemusik?');
            if(ask) {
                document.location = 'form_add_pemusik.php?ibadahId=$tglIbadah'
            }
            else {
...
<head>
    ...
    <title>| FORM JADWAL PEMUSIK</title>
    ...
    <div id="wrapper">
        ...
        <div class="container fluid">
            <div class="page_head" style="margin-bottom: 50px;">
                <h3 style="color: black;">Form Jadwal Pemusik</h3>
                ...
                <p style="display: inline;">&ensp; / &ensp;</p>
                <a href="form_jadwal_pemusik.php" style="display: inline;">Form Jadwal Pemusik</a>
            </div>
            ...
            <div class="container" style="font-family: Poppins; margin-top: 30px;">
                <form style="color: black; padding-bottom: 50px; background-color: white;" method="POST" enctype="multipart/form-data" autocomplete="off">
                    <div class="kiri" style="width: 45%; margin-left: 20px; float:left; margin-right: 35px; padding-top:30px;">
                        <?php
                            $listIbadah = mysqli_query($con, "SELECT DATE_FORMAT(tanggal_ibadah, '%d %M %Y') AS tanggal_ibadah, shift, ibadah_id FROM ibadah WHERE ibadah_id NOT IN (SELECT GROUP_CONCAT(ibadah_id) FROM musik_ibadah GROUP BY ibadah_id)");
                        ?>
                        ...
                        <?php
                            $listPemusik = mysqli_query($con, "SELECT jemaat_id, fullname_jemaat FROM jemaat ORDER BY fullname_jemaat ASC");
                        ?>
