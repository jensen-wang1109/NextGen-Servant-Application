...
<?php 
    ...
    if(isset($_POST['remove'])) {
        ...
        mysqli_query($con, "DELETE FROM musik_ibadah WHERE ibadah_id = '$paramIbadahId' AND pemusik_id = '$namaPemusik'");
        
        mysqli_query($con, "DELETE FROM pemusik WHERE pemusik_id = '$namaPemusik'");

        echo "<script>
            var ask = confirm('Apakah ingin remove pemusik lagi?');
            if(ask) {
                document.location = 'form_remove_pemusik.php?ibadahId=$paramIbadahId'
            }
            else {
...
<head>
    ...
    <title>| FORM REMOVE PEMUSIK</title>
...
<body id="page-top" style="font-family: Poppins;">
    <div id="wrapper">
        ...
        <div class="container fluid">
            <div class="page_head" style="margin-bottom: 50px;">
                <h3 style="color: black;">Form Remove Pemusik</h3>
                ...
                <p style="display: inline;">&ensp; / &ensp;</p>
                <a href="form_remove_pemusik.php?ibadahId=<?php echo $paramIbadahId; ?>" style="display: inline;">Form Remove Pemusik</a>
            </div>

            <div class="clean"></div>

            <div class="row" style="font-family: Poppins; float:left; margin-right:20px;" id="tambah">
                <a class="btn btn-danger" href="jadwal_pemusik.php" onclick="return confirm('Apakah yakin ingin membatalkan proses penghapusan data?');"  role="button"><i class="fas fa-minus"></i>&ensp;Cancel</a>
            </div>
            ...
            <div class="container" style="font-family: Poppins; margin-top: 30px;">
                <form style="color: black; padding-bottom: 50px; background-color: white;" method="POST" enctype="multipart/form-data" autocomplete="off">
                    <div class="kiri" style="width: 45%; margin-left: 20px; float:left; margin-right: 35px; padding-top:30px;">
                        ...
                        <?php
                            $listPemusik = mysqli_query($con, "SELECT p.pemusik_id, j.fullname_jemaat, jp.nama_jenis_pemusik FROM pemusik p JOIN musik_ibadah mi ON p.pemusik_id = mi.pemusik_id JOIN ibadah i ON mi.ibadah_id = i.ibadah_id JOIN jenis_pemusik jp ON mi.jenis_pemusik_id = jp.jenis_pemusik_id JOIN jemaat j ON j.jemaat_id = p.jemaat_id WHERE i.ibadah_id = '$paramIbadahId' ORDER BY j.fullname_jemaat ASC");
                        ?>
                        ...
                        <div class="input-group mb-3" style="padding-bottom: 10px;">
                            <select class="form-select" name="inputnamapemusik" aria-label="Default select example" id="listpemusik" style="color: black;" required>
                                ...
                                ?>
                                <option value="<?php echo $rowListPemusik['pemusik_id']; ?>">
                                    <?php echo $rowListPemusik['fullname_jemaat']; ?> - <?php echo $rowListPemusik['nama_jenis_pemusik']; ?>
                                </option>
                                <?php } ?>
                            </select>   
                        </div>
                    ...
                    <center style="background-color: white; margin-top: 50px;">
                        <input type="submit" class="btn btn-danger" value="Remove Pemusik" name="remove" onclick="return confirm('Yakin menghapus pemusik?')">
                    </center>
                </form>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function() { 
            $('#listpemusik').select2({
                allowClear: true,
                placeholder: "Pilih Nama Pemusik yang Dihapus"
            });
        });
    </script>
