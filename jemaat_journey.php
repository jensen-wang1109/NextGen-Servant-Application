...
<?php
    ...
    $fullJemaat = mysqli_query($con, "SELECT j.*, DATE_FORMAT(j.dob_jemaat, '%d %M %Y') AS dob_jemaat, jj.*, ja.*, j.jemaat_id AS jemaatId FROM jemaat j LEFT JOIN jabatan_jemaat jj ON j.jemaat_id = jj.jemaat_id LEFT JOIN jabatan ja ON jj.jabatan_id = ja.jabatan_id WHERE j.jemaat_id = '$paramJemaatId'");
    $rowFullJemaat = mysqli_fetch_array($fullJemaat);
?>
<head>
    <meta charset="utf-8">
    ...
    <div id="wrapper">
        ...
        <div class="container fluid">
            <div class="page_head" style="margin-bottom: 50px;">
                <h3 style="color: black;"><?php echo $rowFullJemaat['fullname_jemaat']; ?></h3>
                ...
                <p style="display: inline;">&ensp; / &ensp;</p>
                <a href="jemaat_journey.php?jemaatId=<?php echo $rowFullJemaat['jemaatId']; ?>" style="display: inline;">Journey Jemaat</a>
            </div>

            <div class="row" style="font-family: Poppins; float:left; margin-right:20px;">
                <a class="btn btn-danger" href="jemaat.php" role="button"><i class="fas fa-arrow-left"></i>&ensp;Back</a>
            </div>

            <div class="row" style="font-family: Poppins; float:left; margin-right:20px;" id="tambah">
                <a class="btn btn-success" href="form_jemaat_journey.php?jemaatId=<?php echo $rowFullJemaat['jemaatId']; ?>" role="button"><i class="fas fa-plus"></i>&ensp;Add Journey Jemaat</a>
            </div>
                    ...
                    <table id="table" class="table table-hover table-striped" style="color:black;">
                        <thead style="text-align: center;">
                            <tr>
                                ...
                                <th scope="col">Tanggal Mulai Jabatan</th>
                        ...
                        <?php 
                            ...
                            $query = mysqli_query($con, "SELECT ja.nama_jabatan, jj.*, DATE_FORMAT(jj.start_date, '%d %M %Y') AS start_date, DATE_FORMAT(jj.end_date, '%d %M %Y') AS end_date FROM jemaat j JOIN jabatan_jemaat jj ON j.jemaat_id = jj.jemaat_id JOIN jabatan ja ON jj.jabatan_id = ja.jabatan_id WHERE j.jemaat_id = '$paramJemaatId'");
                            ...
                            while($row = mysqli_fetch_array($query)) {
                                ...
                                <tr scope="row">
                                    ...
                                    <td style="text-align: center; width:10%;" class="ed">
                                        <a href="form_jemaat_journey_edit.php?jemaatId=<?php echo $row['jemaat_id']; ?>&jabatanId=<?php echo $row['jabatan_id']; ?>" class="btn btn-warning btn-sm" title="Edit">
                                        <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <a href="jemaat_journey_delete.php?jemaatId=<?php echo $row['jemaat_id']; ?>&jabatanId=<?php echo $row['jabatan_id']; ?>" onclick="return confirm('Yakin menghapus data?')" class="btn btn-danger btn-sm" title="Delete"><i class="fas fa-trash-alt"></i>
                                        </a>
                                    </td>
                                </tr>
